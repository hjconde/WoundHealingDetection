...
parser.add_argument('--A_img_path',type=str,default='/data/Skin_MT/testA/') ###testing images with class A are placed into this folder
parser.add_argument('--B_img_path',type=str,default='/data/Skin_MT/testB/') ###testing images with class B are placed into this folder
parser.add_argument('--AB_image_name_label_path',type=str,default='/data/Skin_MT/testAB_img-name_label.txt')
parser.add_argument('--latentAB_save_path',type=str,default='/results/testAB_WR_features_extraction_results.csv')  ###where the wound-related features extracted from the images and the images label are recorded
parser.add_argument('--trained_CAE_gen_model_path',type=str,default='/trained_models/CAE_gen_skin_trained_model.pt') ### the path of the trained CAE (GAN) model
...
#####extract latents(wound-related features)
...
with open(latentAB_save_path, "w", newline="") as csvfile:
    ...
    with torch.no_grad():
        ###get wound-related features from A images
        img_n = 0
        for image_name in os.listdir(A_img_path):
            ...
            print(str(img_n)+'  '+image_name+' wound-related features: '+s_show_str)

        ###get wound-related features from B images
...
print("Latents(wound-related features) extraction finished")
